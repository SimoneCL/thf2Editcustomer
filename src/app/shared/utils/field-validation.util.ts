...
export class FieldValidationUtil {
    ...
    public vldFieldCharacter(field: string, value: string): boolean {
        if (!value || value.trim() === '') {
            this.thfNotification.error({
                message: this.thfI18nPipe.transform(this.literals['fieldVldRequered'], [this.literals[field]])
            });
            return false;
        }
        return true;
    ...
    public vldRangeCharacter(initialField: string, finalField: string, initialValue: string, finalValue: string): boolean {
        ...
        if (initialValue.trim().toUpperCase() > finalValue.trim().toUpperCase()) {
            this.thfNotification.error({
                message: this.thfI18nPipe.transform(this.literals['fieldVldRange'],
    ...
    public vldValueOfNumber(field: string, value: number): boolean {
        if (value && value < 0) {
            this.thfNotification.error(
                { message: this.thfI18nPipe.transform(this.literals['fieldVldNumber'], [value.toString(), this.literals[field]]) });
            return false;
        }
        return true;
    }
    ...
    public vldFieldDate(field: string, value: Date): boolean {
        ...
        if (!this.disclaimerUtil.isValidDate(valDate)) {
            this.thfNotification.error({
                message: this.thfI18nPipe.transform(this.literals['fieldVldDate'], [this.literals[field]])
            });
            return false;
        }
        return true;
    ...
    public vldFieldMultiSelect(field: string, value: Array<any>): boolean {
        if (!value || value.length === 0) {
            this.thfNotification.error({
                message: this.thfI18nPipe.transform(this.literals['fieldVldSelected'], [this.literals[field]])
            });
            return false;
        }
        return true;
    ...
    public vldFieldFormat(field: string, value: string, format: string, replacePoint = false): boolean {
        ...
        if (!lOK) {
            this.thfNotification.error({
                message: this.thfI18nPipe.transform(this.literals['fieldVldFormat'], [this.literals[field], format])
            });
        }
